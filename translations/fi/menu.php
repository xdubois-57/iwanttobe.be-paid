<?php
// Finnish menu translations for QR Transfer application
// Contains all navigation and header-related text
// Last updated: 2025-04-13

return [
    // Application name and main menu items
    'app_name' => '<strong>Paid!</strong>',
    'menu_home' => 'Koti',
    'menu_about' => 'Tietoa',
    'menu_gdpr' => 'GDPR',
    'menu_why_us' => 'Miksi me?',
    'menu_support' => 'Osta minulle kahvia',
    
    // Language and theme settings
    'language' => 'Kieli',
    'theme' => 'Teema',
    'theme_light' => 'Vaalea',
    'theme_dark' => 'Tumma',
    'theme_auto' => 'Automaattinen',
    
    // Page titles
    'welcome_message' => 'Tervetuloa <strong>Paid!</strong>',
    'about_title' => 'Tietoa <strong>Paid!</strong>',
    'gdpr_title' => 'Tietosuojakäytäntö ja GDPR',
    'why_us_title' => 'Miksi valitset <strong>Paid!</strong>?',
    
    // SEO metadata
    'meta_title' => 'Paid! Ilmainen maksu-QR yhdistyksille',
    'meta_description' => 'Luo QR-koodit skauttapahtumiin, vanhempien yhdistyksiin, jalkapallojoukkueisiin ja pienyrityksiin. Ilmainen, turvallinen ja yksityinen SEPA-maksugeneraattori, joka toimii välittömästi kaikilla laitteilla.',
    'meta_keywords' => 'skauttapahtumat, vanhempien yhdistykset, jalkapallojoukkueet, paikallinen rahoituskampanja, paikalliset tapahtumat, QR-koodi, SEPA-maksu, pankkitili, maksu QR, Eurooppalainen maksu, turvallinen maksu, lahjoitus, tuki, osta minulle kahvia, ilmais QR-koodigeneraattori',
    
    // Legal disclaimer
    'disclaimer_text' => '<strong>Paid!</strong> ei ole vastuussa palvelun käytöstä aiheutuvista rahoituksen ongelmista tai menetyksistä. Tämän työkalun käyttö on omalla vastuulla. Kaikki maksut käsittelevät vastaavat pankit, ja <strong>Paid!</strong> ei käsittele mitään rahoituksellisia transaktioita. Tämä verkkosivusto on lisensoitu <a href="https://www.gnu.org/licenses/gpl-3.0.html">GNU General Public License v3.0 (GPLv3)</a> lisenssin alaisuudessa.',
];
